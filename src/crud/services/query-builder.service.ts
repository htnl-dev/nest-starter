...
export class QueryBuilderService {
   ...
   */
  buildQuery<TDocument>(
    query: QueryDto<TDocument>,
  ): QueryBuildResult<TDocument> {
    const mongoQuery = this.buildMongoQuery(query);
    const sortOptions = this.buildSortOptions(query.sort);
    const { skip, limit } = this.buildPagination(query.page, query.limit);
  ...
  buildMongoQuery<TDocument>(
    ...
    this.convertObjectIdValues(result);
    this.applyTextSearch(result, search);
    this.applyDateFilters(result, createdAfter, createdBefore);
   ...
   */
  async executePaginatedQuery<TDocument>(
  ...
  private applyFilters<TDocument>(
    mongoQuery: FilterQuery<TDocument>,
  ...
  private convertObjectIdValues<TDocument>(
    mongoQuery: FilterQuery<TDocument>,
  ): void {
  ...
  private applyTextSearch<TDocument>(
    mongoQuery: FilterQuery<TDocument>,
    search?: string,
  ): void {
    if (!search) return;
  ...
  private applyDateFilters<TDocument>(
    mongoQuery: FilterQuery<TDocument>,
    createdAfter?: string,
